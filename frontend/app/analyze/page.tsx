...
function fallbackInvestment(report: Record<string, unknown>): Record<string, unknown> {
  ...
  return {
    allocation,
    risk_alignment: `Fallback allocation reflects a ${risk} profile and available investment horizon. Past performance does not guarantee future results.`,
    rationale: "Investment strategy was missing from the completed report; deterministic fallback shown.",
    account_optimization:
      "Use tax-advantaged retirement accounts for higher-growth and income-producing assets when available; keep cash needs liquid and taxable holdings tax-efficient.",
    rebalancing_guidance:
      "Review quarterly and rebalance when any asset class drifts more than 5 percentage points.",
    market_context: {},
    fallback: true,
  };
}
...
export default function AnalyzePage() {
  ...
  // Read from canonical final_report schema (overview key, etc.)
  ...
  // Merge deterministic stress scenarios with critic verdict (status + revision count)
  const stressData: Record<string, unknown> = stressBase.scenarios
    ? {
        ...stressBase,
        status:     (criticData.status as string) ?? "",
        iterations: Number(criticData.revision_count ?? 0) + 1,
      }
  ...
  return (
    <main className="min-h-screen bg-[#0A0F1E] pb-16">
      ...
      <div className="max-w-4xl mx-auto px-4 pt-6 space-y-6">
          ...
          <div className="bg-red-500/8 border border-red-500/25 rounded-2xl p-5">
            <div className="flex items-start gap-3">
              ...
              <button
                ...
                className="inline-flex items-center gap-2 rounded-lg bg-amber-500 px-3 py-2 text-xs font-semibold text-gray-950 hover:bg-amber-400 disabled:opacity-40"
              >
                <RotateCcw size={12} />
                Retry
              </button>
        ...
        {showPipeline && (
          <AgentPipeline agents={analysis.agents} revisionCount={analysis.revisionCount} />
        )}

        {showDashboard && (
          <div ref={resultsRef} className="scroll-mt-24 space-y-5">
            {/* Health score hero + download */}
            <div className="rounded-2xl border border-amber-500/15 bg-gray-900/50 p-6">
              <div className="flex items-center justify-between">
                ...
                <div className="flex items-center gap-3">
                  <button
                    onClick={() => { analysis.reset(); setLoadedProfile(null); setActivePersona(null); }}
                    className="inline-flex items-center gap-2 rounded-xl border border-gray-700 px-4 py-2.5 text-xs font-semibold text-gray-300 hover:border-gray-500 hover:text-white transition-colors"
                  >
                    <RotateCcw size={14} />
                    New Analysis
                  </button>
                  <button
                    ...
                    className="inline-flex items-center gap-2 rounded-xl bg-amber-500 px-4 py-2.5 text-xs font-semibold text-gray-950 hover:bg-amber-400 disabled:opacity-50 transition-colors"
                  >
                    <Download size={14} />
            ...
            <div className="flex overflow-x-auto border-b border-gray-800 bg-gray-900/40 rounded-t-2xl scrollbar-none">
              {DASH_TABS.map((tab) => (
                <button
                  ...
                  className={`min-w-[80px] flex-1 text-xs py-3 font-ui transition-colors whitespace-nowrap ${
                    dashTab === tab.id
            ...
            <div className="bg-gray-900/40 border border-gray-800 rounded-b-2xl rounded-tr-2xl p-5">
              ...
              {dashTab === "compliance"  && (
                <ComplianceReport
                  ...
                  pipelineErrors={result?.pipeline_errors}
                />
              )}
            </div>
